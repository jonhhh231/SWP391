    ...
    public WastageResponse recordWastage(WastageRequest request) {
        ...
        ProductionRun run = productionRunRepository
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                        "Không tìm thấy phiên sản xuất đang chế biến với ID: " + request.getRunId()));

        // 2. Validate waste không vượt quá planned
        ...
        if (newWaste.compareTo(run.getPlannedQty()) > 0) {
            throw new IllegalArgumentException(
                    "Số lượng hao hụt không được vượt quá số lượng kế hoạch: " + run.getPlannedQty());
        }
        ...
        // 4. Trừ kitchen_stock của các ingredient liên quan
        List<Ingredient> ingredients = ingredientRepository
                .findByFormulas_Product_ProductId(run.getProduct().getProductId());

        for (Ingredient ingredient : ingredients) {
            BigDecimal deduct = request.getWasteQty(); // có thể nhân với amount_needed nếu cần
            BigDecimal newStock = ingredient.getKitchenStock().subtract(deduct);
